...
*/
	...
	class Dashboard
	{
		...
		function getDataForQuestion($question_id = 0, $age_filter='none', $gender_filter='none', $time_filter='none') {
			...
			switch($time_filter){
				case '3h':
					$time_filter_query = ' and cast((time_to_sec(timediff(now(), r.response_date))/60) as decimal(5,0))/60 < 3 ';
				break;
				case '6h':
					$time_filter_query = ' and cast((time_to_sec(timediff(now(), r.response_date))/60) as decimal(5,0))/60 < 6 ';
				break;
				case '24h':
					$time_filter_query = ' and cast((time_to_sec(timediff(now(), r.response_date))/60) as decimal(5,0))/60 < 24 ';
				break;
				case '7d':
					$time_filter_query = ' and datediff(now(), r.response_date) <= 7 ';
				break;
				case '30d':
					$time_filter_query = ' and datediff(now(), r.response_date) <= 30 ';
				break;
			}
		...
		function getDataForEstimatedTime($time_filter = 'none'){
			$data = array();
		...
		function getDetailsForEstimatedTime($time_filter = 'none'){
			$data = array();
			...
			$sql = "SELECT  avg(etime) as 'etime', avg(atime) as 'atime', date(order_date), truncate(last_hours, 0) as 'last_hours'
						 FROM (
							SELECT restaurant_id, order_key, max(served_date), max(order_date) as 'order_date', max(estimated_d_time) as 'etime', 
										cast((time_to_sec(timediff(max(served_date), max(order_date)))/60) as decimal(5,0)) as 'atime',  
										timediff(max(served_date), max(order_date)),
										cast((time_to_sec(timediff(now(), max(order_date)))/60) as decimal(5,0))/60 as 'last_hours'
							FROM restaurants.tbl_orders
							WHERE order_status_id > 2 AND estimated_d_time IS NOT NULL
							GROUP BY order_key 
						 ) x
			...
			switch($time_filter){
				case '3h':
					$sql .= " and last_hours < 3
								GROUP BY truncate(last_hours,0)
								ORDER BY last_hours";
					break;
				case '6h':
					$sql .= " and last_hours < 6
								GROUP BY truncate(last_hours,0)
								ORDER BY last_hours";
					break;
				case '24h':
					$sql .= " and last_hours < 24
								GROUP BY truncate(last_hours,0)
								ORDER BY last_hours";
					break;
				case '7d':
					$sql .= " and datediff(now(), order_date) <= 7
								GROUP BY day(order_date)
								ORDER BY order_date";
					break;
				case '30d':
					$sql .= " and datediff(now(), order_date) <= 30
								GROUP BY day(order_date)
								ORDER BY order_date";
					break;
