	...
	if(isset($_POST['m'])){
		if($_POST['m'] == 'new'){
			...
			if($newid > 0){
				...
				$_SESSION['feedback'] = '<div class="alert alert-dismissible alert-danger" style="margin-top:50px">
																<button type="button" class="close" data-dismiss="alert">&times;</button>
																'.$newid.'
															  </div>';
				header('Location: index.php?a=cms&s=menu-view&mid='.$_GET['mid']);
...
?>
		<div class="section-header">
			<h1 class="section-title wow fadeInRight" data-wow-duration="1000ms" data-wow-delay="300ms">
				<a href="index.php?a=cms&s=menu-view&mid=<?php echo ((isset($_GET['mid']))?$_GET['mid']:$item['menu_id']); ?>u" class="btn btn-fab btn-fab-mini btn-flat btn-primary" data-toggle="tooltip" title="Go Back to Menu"><i class="fa fa-arrow-left"></i></a>
				<?php echo (($iid > 0)? 'Edit Menu Item': 'New Menu Item'); ?>
			</h1>
			<hr>
        </div>
